...
class Board:
    def __init__(self, columns, rows, square_size, width, height):
        ...
        self.squares = [
            [BLOCK_COLOR for column in range(self.columns)] for row in range(self.rows)
        ]
    ...
    def draw_title(self, text, surface):
        ...
        surface.blit(
            ...
            (
                ...
                self.start_y // 2 - title_text.get_height() / 2,
            ),
        )
    ...
    def draw_sides_text(self, surface, player, format_timer, get_max_score):
        ...
        surface.blit(
            score_text,
            (
                self.start_x // 2 - score_text.get_width() / 2,
                self.screen_height // 4 - score_text.get_height() / 2,
            ),
        )

        max_score_text = SIDE_FONT.render(
            f"Max score: {get_max_score() if get_max_score() else 0}", True, TEXT_COLOR
        )
        surface.blit(
            max_score_text,
            (
                self.screen_width - self.start_x // 2 - max_score_text.get_width() / 2,
        ...
        surface.blit(
            timer_text,
            (
                self.start_x // 2 - timer_text.get_width() / 2,
                self.screen_height // 4 - timer_text.get_height() / 2 + 100,
            ),
        )

        speed_text = SIDE_FONT.render(
            f"Speed: {round(player.speed, 1)}", True, TEXT_COLOR
        )
        surface.blit(
            speed_text,
            (
                self.screen_width - self.start_x // 2 - speed_text.get_width() / 2,
        ...
        for i in range(len(self.squares)):
            for j in range(len(self.squares[i])):
                pygame.draw.rect(
                    ...
                    self.squares[i][j],
                    (
                        self.start_x + j * self.square_size,
    ...
    def draw_grid(self, surface):
        for line in range(self.rows + 1):
            pygame.draw.line(
                surface,
                GRID_COLOR,
                (self.start_x, self.start_y + line * self.square_size),
                (
                    self.start_x + self.rows * self.square_size,
                    self.start_y + line * self.square_size,
                ),
                3,
            )
        for line in range(self.columns + 1):
            pygame.draw.line(
                surface,
                GRID_COLOR,
                (self.start_x + line * self.square_size, self.start_y),
                (
                    self.start_x + line * self.square_size,
                    self.start_y + self.columns * self.square_size,
                ),
                3,
            )
    ...
    def draw_name(self, surface, player, board, main):
        ...
        while draw:
            ...
            surface.blit(
                lost_text,
                (
                    self.screen_width / 2 - lost_text.get_width() / 2,
            ...
            surface.blit(
                input_text,
                (
                    self.screen_width / 2 - input_text.get_width() / 2,
    ...
    def draw_lost_text(self, surface, player, main):
        ...
        while lost:
            ...
            retry_text = SIDE_FONT.render(
                "Do you want to play again?", True, TEXT_COLOR
            )
            surface.blit(
                retry_text,
                (
                    self.screen_width / 2 - retry_text.get_width() / 2,
            ...
            for i, v in enumerate(retry_options, start=1):
                ...
                surface.blit(
                    label,
                    (
                        self.screen_width / 2 - label.get_width() / 2 - v[1],
